...
class AnalyzeService:
    ...
    async def get_top10_articles_service(self) -> List[Dict[str, Any]]:
        articles = None
        # 1. 优先 Hive
        try:
            articles = await self.articleMapper.get_top10_articles_hive_mapper()
        ...
        if not articles or len(articles) == 0:
            try:
                articles = await self.articleMapper.get_top10_articles_spark_mapper()
                if articles and isinstance(articles[0], dict):
        ...
        # 3. DB 兜底
        if not articles or len(articles) == 0:
            articles = await self.articleMapper.get_top10_articles_db_mapper()
        ...
        if articles and isinstance(articles[0], dict):
            ...
            users = await self.userMapper.get_users_by_ids_mapper(user_ids)
    ...
    async def get_keywords_dic(self) -> Dict[str, int]:
        all_keywords: List[str] = await self.articleLogMapper.get_search_keywords_articlelog_mapper()
