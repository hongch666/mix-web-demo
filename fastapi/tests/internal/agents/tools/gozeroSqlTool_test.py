"""GozeroSqlTool 远程 SQL 代理工具的单元测试"""
...
from app.internal.agents.tools.gozeroSqlTool import GozeroSqlTool
...
def _tool() -> tuple[GozeroSqlTool, AsyncMock]:
    client = AsyncMock()
    return GozeroSqlTool(client), client
...
async def test_get_tables_wraps_client_failure() -> None:
    ...
    assert await tool.get_tables("t") == Messages.SQL_TOOL_TABLE_SCHEMA_FAILED(
        "GoZero", RuntimeError("remote down")
    )
...
def test_get_langchain_tools_exposes_table_and_query_tools() -> None:
    ...
    assert [item.name for item in tool.get_langchain_tools()] == [
        Messages.SQL_TOOL_GOZERO_TABLE_TOOL_NAME,
        Messages.SQL_TOOL_GOZERO_QUERY_TOOL_NAME,
    ]
