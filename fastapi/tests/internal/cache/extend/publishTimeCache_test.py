...
from app.internal.cache.extend import publishTimeCache as module
...
def cache(
...
) -> tuple[module.PublishTimeCache, _FakeRedis]:
    ...
    return module.PublishTimeCache(), fake_redis


# PublishTimeCache 常量指向月度发布量数仓表，L1 TTL 为 300 秒
def test_cache_constants_target_monthly_publish_warehouse_table(
    cache: tuple[module.PublishTimeCache, _FakeRedis],
) -> None:
    instance, _ = cache

    assert instance.REDIS_KEY_PREFIX == RedisKeys.PUBLISH_MONTHLY_COUNT
    assert instance.REDIS_VERSION_KEY == RedisKeys.PUBLISH_MONTHLY_COUNT_VERSION
    assert instance.L1_CACHE_TTL == 300
    assert instance.VERSION_MODEL is AdsMonthlyPublish
...
async def test_get_returns_local_data_on_l1_hit(
    ...
    instance, _ = cache
    await instance.update_local_cache([{"month": "2026-09"}])
    monkeypatch.setattr(instance, "is_version_changed", AsyncMock(return_value=False))

    assert await instance.get() == [{"month": "2026-09"}]
...
async def test_get_reads_redis_and_warms_local_on_l1_miss(
    ...
    monkeypatch.setattr(instance, "is_version_changed", AsyncMock(return_value=False))
    fake_redis.get.return_value = [{"month": "2026-10"}]

    assert await instance.get() == [{"month": "2026-10"}]

    fake_redis.get.assert_awaited_once_with(RedisKeys.PUBLISH_MONTHLY_COUNT)
    assert instance._local_cache == [{"month": "2026-10"}]
...
async def test_set_writes_both_levels_and_commits_version(
    ...
    await instance.set([{"month": "2026-09"}])

    update_local.assert_awaited_once_with([{"month": "2026-09"}])
    update_redis.assert_awaited_once_with([{"month": "2026-09"}])
    update_version.assert_awaited_once()


# 工厂函数返回经 lru_cache 缓存的同一 PublishTimeCache 实例
def test_factory_returns_lru_cached_singleton(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_redis = _FakeRedis()
    monkeypatch.setattr(base_cache, "get_redis_client", lambda: fake_redis)
    monkeypatch.setattr(module, "_publish_time_cache_instance", None)
    module.get_publish_time_cache.cache_clear()
    try:
        first = module.get_publish_time_cache()
        second = module.get_publish_time_cache()

        assert isinstance(first, module.PublishTimeCache)
