...
from app.internal.agents import IntentRouter, UserPermissionManager
...
class GeminiService(BaseAiService):
    ...
    async def simple_chat(
        self, message: str, user_id: int = 0, db: Optional[Session] = None
    ) -> str:
        """增强的聊天接口 - 使用Agent和RAG"""
        try:
    ...
    async def stream_chat(
        ...
        try:
                ...
                try:
                    ...
                    if "ResourceExhausted" in error_msg or "429" in error_msg:
                        if "quota" in error_msg.lower():
                            yield {
                                "type": "content",
                                "content": Constants.GEMINI_QUOTA_EXCEEDED_ERROR,
                            }
                        else:
                            yield {
                                "type": "content",
                                "content": Constants.GEMINI_RATE_LIMIT_EXCEEDED_ERROR,
                            }
                    elif "invalid" in error_msg.lower() and "key" in error_msg.lower():
                        yield {
                            "type": "content",
                            "content": Constants.GEMINI_INVALID_API_KEY_ERROR,
                        }
                    else:
                        yield {
                            "type": "content",
                            "content": f"服务异常: {error_msg[:100]}",
                        }
                return
            ...
            if intent == "general_chat":
                ...
                try:
                    ...
                    if "ResourceExhausted" in error_msg or "429" in error_msg:
                        ...
                        yield {
                            "type": "content",
                            "content": f"AI 处理失败: {error_msg[:100]}",
                        }
                    return
            ...
            if "ResourceExhausted" in error_msg or "429" in error_msg:
                ...
                "API_KEY_INVALID" in error_msg or "invalid API key" in error_msg.lower()
            ):
                yield {
                    "type": "content",
                    "content": Constants.GEMINI_INVALID_API_KEY_ERROR,
