...
class KnowledgeGraphSyncService:
    ...
    async def _cleanup_write(
        ...
        # 会等价于 `WHERE true`，从而误删图中全部对应关系/节点，这通常意味着上游快照抓取失败（返回空），属于异常情况，必须跳过而非执行全量删除
        ...
        if "keys" in params:
            ...
            """
            节点类清理（`DETACH DELETE n`，通过 `WHERE NOT n.id IN $ids` 删节点）：语义是"删除不在该集合中的所有节点"，若分批切小 $ids，每批都会误删集合之外的全部节点（灾难+OOM），因此必须一次传入完整集合，确保只删除真正不存在的节点，若集合为空则由上面的保护直接跳过
            """
    ...
    async def _cleanup_deleted_graph_data(
        ...
        # 直接复用调用方传入的归一化快照（同步任务第一阶段已抓取并归一化），不再二次远程抓取，避免并发占用 spring R2DBC 连接池导致超时
        users = users or []
