...
def export_articles_to_csv_and_hive(
    ...
    if article_mapper is None:
        from app.crud import get_article_mapper
    ...
    if article_cache is None:
        try:
            from app.cache import get_article_cache
    ...
    if category_cache is None:
        try:
            from app.cache import get_category_cache
    ...
    if publish_time_cache is None:
        try:
            from app.cache import get_publish_time_cache
    ...
    if statistics_cache is None:
        try:
            from app.cache import get_statistics_cache
