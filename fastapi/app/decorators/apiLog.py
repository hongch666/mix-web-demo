...
def _extract_request_body_for_queue(
    ...
    try:
        ...
        for key, value in kwargs.items():
            ...
            if any(suffix in key for suffix in exclude_type_suffixes) or any(
                suffix in type(value).__name__ for suffix in exclude_type_suffixes
            ):
                continue
            ...
            if key in sig.parameters:
                ...
                if param_annotation != inspect.Parameter.empty:
                    ...
                    if any(
                        t in annotation_str
                        for t in ["Query", "Path", "Header", "Cookie", "Form"]
                    ):
                        continue
...
def _normalize_path_with_params(path: str, request: Optional[Request]) -> str:
    ...
    for param_name, param_value in request.path_params.items():
        ...
        normalized_path = normalized_path.replace(
            f"/{param_value_str}", f"/:{param_name}"
        )
