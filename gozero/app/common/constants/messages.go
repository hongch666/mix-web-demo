...
const (
	...
	// pgvector 向量同步
	TASK_EMBEDDING_SYNC_START_LOG     = "开始同步文章向量到 ES，共 %d 篇"
	TASK_EMBEDDING_SYNC_DONE_LOG      = "向量同步完成: 成功 %d, 跳过 %d"
	TASK_EMBEDDING_SYNC_PGVECTOR_FAIL = "文章 %d 读取 pgvector 失败: %v"
	TASK_EMBEDDING_SYNC_ES_FAIL       = "文章 %d 写入 ES embedding_vector 失败: %v"
	TASK_EMBEDDING_SYNC_CONNECT_FAIL  = "连接 pgvector 失败: %w"
	TASK_EMBEDDING_SYNC_ARTICLES_FAIL = "获取已发布文章列表失败: %w"

	// Neo4j 图谱缓存同步
	TASK_GRAPH_CACHE_SYNC_START_LOG   = "开始同步 %d 个活跃用户的图谱特征"
	TASK_GRAPH_CACHE_SYNC_DONE_LOG    = "图谱特征同步完成，共 %d 个用户"
	TASK_GRAPH_CACHE_CONNECT_FAIL     = "连接 Neo4j 失败: %w"
	TASK_GRAPH_CACHE_ACTIVE_USER_FAIL = "查询活跃用户失败: %w"

	// 手动同步日志
	TASK_EMBEDDING_SYNC_MANUAL_START = "开始手动触发文章向量同步（pgvector 均值 → ES）"
	TASK_EMBEDDING_SYNC_MANUAL_FAIL  = "文章向量同步失败"
	TASK_EMBEDDING_SYNC_MANUAL_DONE  = "文章向量同步完成"
	TASK_GRAPH_CACHE_MANUAL_START    = "开始手动触发表谱特征缓存同步（Neo4j → Redis）"
	TASK_GRAPH_CACHE_MANUAL_FAIL     = "图谱特征缓存同步失败"
	TASK_GRAPH_CACHE_MANUAL_DONE     = "图谱特征缓存同步完成"

	// 搜索预处理
	SEARCH_PREPARE_FAIL        = "搜索预处理失败"
	SEARCH_EMBED_FAIL          = "嵌入向量获取失败: %v"
	SEARCH_GRAPH_FEATURE_FAIL  = "图谱特征获取失败: %v"
	SEARCH_MATCHED_CHUNKS_FAIL = "获取 matched_chunks 失败: %v"

	// FastAPI 客户端
	FASTAPI_WEIGHTS_FORMAT_ERROR    = "权重响应格式异常"
	FASTAPI_EMBED_FORMAT_ERROR      = "嵌入向量响应格式异常"
	FASTAPI_EMBED_DATA_FORMAT_ERROR = "嵌入向量数据格式异常"

	// 向量匹配原因
	VECTOR_REASON_HIGH   = "搜索词与文章核心内容高度相符"
	VECTOR_REASON_MEDIUM = "搜索词与文章部分内容相关"
	VECTOR_REASON_LOW    = "搜索词与文章存在一定语义关联"

	// 图谱匹配原因
	GRAPH_REASON_FOLLOWED_AUTHOR = "来自你关注的作者 %s"
	GRAPH_REASON_TAG_INTEREST    = "与你最近点赞/收藏过的标签 %s 相似"
	GRAPH_REASON_SUB_CATEGORY    = "属于你常看的分类 %s"
	GRAPH_RELATION_FOLLOWED      = "关注作者"
	GRAPH_RELATION_TAG_INTEREST  = "与你最近点赞/收藏过的标签相似"
	GRAPH_RELATION_TYPE_FOLLOW   = "followed_author"
	GRAPH_RELATION_TYPE_TAG      = "tag_interest"

	// Redis 图谱缓存
	REDIS_GRAPH_CACHE_KEY_PREFIX = "user_graph"
)
