...
const (
	...
	GRPC_CHANNEL_UNAVAILABLE               = "gRPC 通道不可用"
	GRPC_RESPONSE_EMPTY                    = "gRPC 返回为空"
	GRPC_CALL_SUCCESS                      = "gRPC 调用成功：服务=%s，路径=%s"
	GRPC_CALL_FALLBACK                     = "gRPC 调用失败，降级 HTTP：服务=%s，路径=%s，错误=%v"
