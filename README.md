...
16. 基于 NestJS 和 Spring 实现 **GitHub OAuth 登录/注册**，NestJS 处理 GitHub 授权回调并创建/关联用户，Spring 生成站内登录票据，支持首次 GitHub 登录自动注册，前端通过一次性 ticket 换取 JWT
...
| 注册中心 | Nacos                                                                                                                                |
| 运行时   | Python 3.12+                                                                                                                         |
| 主要功能 | 数据分析与统计、搜索算法、RAG 文章检索增强、Neo4j 知识图谱、AI Agent（SQL/MongoDB/向量搜索/图谱查询）、多模型（GPT/Gemini/DeepSeek） |
| 代码位置 | `fastapi/`                                                                                                                         |
...
4. FastAPI 项目：`fastapi/app/core/constants/`
   ...
   - `scripts.py`：脚本类（SQL 语句、Cypher 语句、SQL 安全规则）
