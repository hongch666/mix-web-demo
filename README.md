...
14. 基于 FastAPI 和 LangChain 实现 RAG 文章检索增强和 Tools 调用 SQL、MongoDB、Neo4j，支持 **GPT/Gemini/DeepSeek** 进行多模型选择
15. 基于 FastAPI 和 Neo4j 实现文章、用户、分类、标签、点赞、收藏、评论、关注等实体关系图谱，用于关系查询和图谱推荐
16. 基于 FastAPI 和 LangSmith 实现 LLM 链路可观测性，对 AI 聊天、RAG 检索、Agent Tools 调用和向量同步任务进行全链路 Trace，支持采样率控制与敏感数据脱敏
17. 基于 NestJS 和 Spring 实现 **GitHub OAuth 登录/注册**，NestJS 处理 GitHub 授权回调并创建/关联用户，Spring 生成站内登录票据，支持首次 GitHub 登录自动注册，前端通过一次性 ticket 换取 JWT
...
3. NestJS 项目的非module通用工具放置在 `/common`下，和系统相关的模块放置在 `/framework`下，如 `filters`、`guards`、`interceptors`等，module 相关的工具模块放置在 `/module`下，其中`/system`是放置接口相关模块，`/common`放置通用模块(需要依赖注入的工具模块)
...
- Feign 接口文件使用 `<TargetService>Client.java`
- 降级工厂放 `fallback/` 目录
