...
main() {
    ...
    case $command in
        ...
        up)
            ...
            echo ""
            log_info "数据库访问信息:"
            echo "  MySQL:         localhost:3306 (root/$DB_PASSWORD)"
            echo "  PostgreSQL:    localhost:5432 (postgres/$DB_PASSWORD)"
            echo "  Redis:         localhost:6379 (密码: $REDIS_PASSWORD)"
            echo "  MongoDB:       localhost:27017 (root/$MONGO_PASSWORD)"
