...
"cd spring && [ -f .env ] && export \$(cat .env | grep -v '^#' | xargs) && mvn spring-boot:run" C-m
...
"cd nestjs && [ -f .env ] && export \$(cat .env | grep -v '^#' | xargs) && npm run bun:dev" C-m

# window 4: fastapi
tmux new-window -t $SESSION:4 -n fastapi -c "$WORKDIR"
tmux send-keys -t $SESSION:4 \
"cd fastapi && [ -f .env ] && export \$(cat .env | grep -v '^#' | xargs) && uv run python main.py" C-m

# window 5: gateway
tmux new-window -t $SESSION:5 -n gateway -c "$WORKDIR"
tmux send-keys -t $SESSION:5 \
"cd gateway && [ -f .env ] && export \$(cat .env | grep -v '^#' | xargs) && mvn spring-boot:run" C-m
