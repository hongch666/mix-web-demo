...
public class UserController {
    ...
    public Result listUsers(@ModelAttribute UserQueryDTO queryDTO) {
        ...
        log.info("用户" + userId + ":" + userName + " GET /users: " + "获取用户信息\nUserQueryDTO: {}", queryDTO);
    ...
    public Result addUser(@Valid @RequestBody UserCreateDTO userDto) {
        Long userId = UserContext.getUserId();
        String userName = UserContext.getUsername();
        log.info("用户" + userId + ":" + userName + " POST /users: " + "新增用户\nUserCreateDTO: {}", userDto);
    ...
    public Result deleteUser(@PathVariable Long id) {
        Long userId = UserContext.getUserId();
        String userName = UserContext.getUsername();
        log.info("用户" + userId + ":" + userName + " DELETE /users/{id}: " + "删除用户，ID: {}", id);
    ...
    public Result getUserById(@PathVariable Long id) {
        Long userId = UserContext.getUserId();
        String userName = UserContext.getUsername();
        log.info("用户" + userId + ":" + userName + " GET /users/{id}: " + "查询用户，ID: {}", id);
    ...
    public Result updateUser(@Valid @RequestBody UserUpdateDTO userDto) {
        Long userId = UserContext.getUserId();
        String userName = UserContext.getUsername();
        log.info("用户" + userId + ":" + userName + " PUT /users: " + "修改用户\nUserUpdateDTO: {}", userDto);
        User user = BeanUtil.copyProperties(userDto, User.class);
    ...
    public Result updateUserStatus(@PathVariable Long id, @RequestParam String status) {
        Long userId = UserContext.getUserId();
        String userName = UserContext.getUsername();
        log.info("用户" + userId + ":" + userName + " PUT /users/status/{id}: " + "修改用户状态\nID: {}, 状态: {}", id, status);
    ...
    public Result getUserStatus(@PathVariable Long id) {
        Long userId = UserContext.getUserId();
        String userName = UserContext.getUsername();
        log.info("用户" + userId + ":" + userName + " GET /users/status/{id}: " + "查询用户状态\nID: {}", id);
        String key = "user:status:" + id;
    ...
    public Result logout(@PathVariable Long id) {
        Long userId = UserContext.getUserId();
        String userName = UserContext.getUsername();
        log.info("用户" + userId + ":" + userName + " POST /users/logout/{id}: " + "用户登出\nID: {}", id);
        String key = "user:status:" + id;
        redisUtil.set(key, "0");
        return Result.success();
    }
